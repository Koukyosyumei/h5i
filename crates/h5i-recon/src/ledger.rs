...
// The shapes and the fold live in `h5i-wire`, because the console reads a
// ledger too and must not link a plugin to do it. What stays here is the file:
// where it is, how it is written, and the state beside it.
pub use h5i_wire::ledger::{
    Endpoint, Inventory, MAX_EVIDENCE, MAX_FIELD_BYTES, MAX_READ_BYTES, Observation, Param, Source,
    State, Where, endpoint_id, fold,
};
...
impl Ledger {
    ...
    ///
    /// Bounded, and lossy about lines it cannot read: the fold in `h5i-wire`
    /// counts those rather than guessing, because a killed run leaves a torn
    /// last line.
    pub fn read(&self) -> Result<Inventory, H5iError> {
        ...
        let truncated = text.len() as u64 > MAX_READ_BYTES;
        ...
        let mut inventory = fold(text.lines());
