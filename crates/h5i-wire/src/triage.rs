...
//! with none of them lost (design-recon.md N11, design-websec.md W22).
//!
//! Here rather than in a plugin because two programs fold responses: recon,
//! which asks whether a path exists, and the workbench's experiment, which asks
//! which of its sends came back different. The question differs, the fold does
//! not.
//!
...
pub struct Sample {
    ...
    pub endpoint: String,
    /// What names this sample to a reader: the path recon probed, or the
    /// values an experiment set. Display only; nothing groups by it.
    pub name: String,
...
pub struct Cluster {
    ...
    /// Member names, capped for display.
    pub names: Vec<String>,
}

/// The most member names one cluster lists before it starts counting instead.
pub const MAX_NAMES_SHOWN: usize = 10;

/// Which question the fold is answering.
///
/// Two callers want different grains from the same responses. Recon is folding
/// a wordlist's worth of misses into one row, so two pages built from one
/// template belong together. An experiment is looking for the one send that
/// came back different, and "same shape, same size, different words" is
/// precisely the answer it came for.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum By {
    /// Status, type, redirect target, tag shape, then size. What recon folds.
    Shape,
    /// All of that, and what the document says.
    ShapeAndWords,
}
...
pub fn cluster(samples: &[Sample], by: By) -> Vec<Cluster> {
    ...
    for sample in samples {
        let key = format!(
            ...
            match by {
                By::Shape => "",
                By::ShapeAndWords => sample.text.as_str(),
            },
        );
...
mod tests {
    ...
    fn the_noise_folds_and_the_exceptions_stay_visible() {
        ...
        let clusters = cluster(&samples, By::Shape);
    ...
    /// The experiment's question: same status, same size, different answer.
    #[test]
    fn words_separate_two_responses_that_look_identical_on_the_wire() {
        let mut allowed = sample("id=1", "req_1", 200, 300, "aaaa");
        allowed.text = text_digest("<p>invoice for alice</p>", "/invoice");
        let mut refused = sample("id=2", "req_2", 200, 302, "aaaa");
        refused.text = text_digest("<p>you cannot see that</p>", "/invoice");
        let samples = vec![allowed, refused];

        assert_eq!(
            cluster(&samples, By::Shape).len(),
            1,
            "recon folds a template's renderings together"
        );
        assert_eq!(
            cluster(&samples, By::ShapeAndWords).len(),
            2,
            "an experiment came for exactly this difference"
        );
    }
